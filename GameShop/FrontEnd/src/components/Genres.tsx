...
export const Genres: React.FC = () => {
    const [items, setItems] = useState<GenreResponse[]>([]);
    const [selectedItem, setSelectedItem] = useState<GenreResponse | null>(null);
    ...
    const handleItemClick = async (item: GenreResponse) => {
        try {
            const data = await genreService.getGenreById(item.id);
    ...
    const handleSaveAdd = async () => {
        ...
        try {
            ...
            setAddError(err instanceof Error ? err.message : 'Failed to create genre');
        }
    };
